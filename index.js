...
🤖 \`!ask <คำถาม>\` — ถามคำถามกับ Gemma 4 AI
🎨 \`!image <คำอธิบาย>\` — สร้างรูปภาพ
🔊 \`!speak <ข้อความ>\` — แปลงข้อความเป็นเสียง (Gemini TTS)
...
หรือจะ **mention บอท** / **DM** ตรงๆ ก็ได้เลยครับ!`);
  ...
  if (content.startsWith("!speak ")) {
    ...
    try {
      ...
      const filePath = path.join("/tmp", `tts_${Date.now()}.wav`);
      fs.writeFileSync(filePath, audioBuffer);

      const attachment = new AttachmentBuilder(filePath, { name: "speech.wav" });
